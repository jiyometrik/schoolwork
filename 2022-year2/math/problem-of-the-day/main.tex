...
\begin{questions}
	...
	\( \dfrac{a^4-a^2b^2}{(a-b)^2} \div \frac{a(a+b)}{b^2} \times \frac{b}{a} \).
	...
	\question (\textbf{27 May}) Which of the following is a possible plot of the two lines \(l_1\): \(y = (k - 2)x + k\) and \(l_2\): \(y = kx\)?
	\begin{figure}[htpb]
	    \centering
	    \includegraphics{images/0527_Graphs.png}
	    \caption{Four plots.}
	    \label{fig:0527_Graphs}
	\end{figure}
	
	\begin{solution}
	\textbf{B}.
	\begin{itemize}
	    \item \textbf{A} cannot be a possible plot as \(l_1\) would intersect the negative \(x\)-axis when \(k < 0\).
	    \item \textbf{B} can be a possible plot if \(k = 1\).
	    \item \textbf{C} cannot be a possible plot as \(l_1\) would intersect \(l_2\) in the 1st Quadrant if \(k \geq 3\).
	    \item \textbf{D} cannot be a possible plot as \(l_1\) would intersect \(l_2\) in the 2nd Quadrant if \(k < 0\).
	\end{itemize}
	\end{solution}
\end{questions}
\end{document}
