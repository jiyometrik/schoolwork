...
\begin{marginfigure}
	% \centering
	\includegraphics[width=.8\textwidth]{assets/twopulleys.png}
...
\begin{itemize}
	...
	\item \({v}_b\) and \({v}_p\) to be the velocities of the ball
...
\begin{marginfigure}
	%\centering
	\includegraphics[width=\textwidth]{assets/momentumgraph.png}
	\caption{Momentum--time graph for two carts}
	\label{fig:twocarts}
\end{marginfigure}
\begin{problem}
\Cref{fig:twocarts} shows the momentum--time graph for a
collision between
...
\begin{align*}
	\Delta{p}_1        & = -\Delta{p}_2    \\
	\Rightarrow {J}_1  & = -{J}_2          \\
	\odv{{J}_1}{t}     & = -\odv{{J}_2}{t} \\
	\therefore\; {F}_1 & = -{F}_2
\end{align*}
...
\begin{marginfigure}
	% \centering
	\includegraphics[width=0.65\textwidth]{assets/puttylump.png}
...
\begin{itemize}
	\item \(m_f\) and \(m_p\) are the masses of the frame and putty
	      lump respectively.
	\item \({u}_p\) is the velocity of the putty lump just before
	      hitting the frame.
	\item \({v}\) is the velocity of the putty lump and frame
...
\begin{align*}
	m_p{u}_p + 0 & = {v}\ab(m_p + m_f)       \\
	{v}          & = \f{m_p{u}_p}{m_p + m_f}
\end{align*}

The gravitational potential energy lost due to the descent and the
kinetic energy
gained will contribute to the stretching of the spring. Taking \({s}\) to be
the maximum length of the spring,
\begin{align*}
	m{g}\ab({s}-x_0) + \f{1}{2}\ab(m_p + m_f){v}^2 & =
	\f{1}{2}k{s}^2                                                               \\
	\f{k}{2}{s}^2 - m{g}{s} + m{g}x_0 -
	\f{{v}^2\ab(m_p+m_f)}{2}                       & = 0                         \\
	{s}                                            & = \qty{0.340}{\metre}       \\
	\therefore\;{s} - x_0                          & = \hil{\qty{0.270}{\metre}}
\end{align*}
...
\begin{align*}
	\Delta{p} \text{ (per second)} & = m\Delta{v}                              \\
	                               & = 0.100 \times 4.43                       \\
...
\end{problem}
We know that \({F} = \odv{{p}}{t}\). therefore, \({F} =
\lf{\Delta{p}}{\Delta t} = \hil{\qty{0.443}{\newton}}\).
...
Resolving the velocity vectors in the horizontal and vertical directions respectively,
\begin{align}
	\label{eq:pconvx}
	m_1{u} & = m_1{v}_1 \cos\alpha + m_2{v}_2 \cos \beta  \\
	0      & = m_1{v}_1 \sin \alpha - m_2{v}_2 \sin \beta
	\label{eq:pconvy}
\end{align}
...
\begin{equation}
	{u} = {v}_2 \cos \beta - {v}_1 \cos \alpha
	\label{eq:approachspeed}
\end{equation}

We can rewrite \cref{eq:pconvy} as
\begin{equation}
	m_2{v}_2 = \dfrac{m_1{v}_1\sin\alpha}{\sin\beta}
	\label{eq:mv2}
\end{equation}

Substituting the result from \cref{eq:mv2} into \cref{eq:pconvx}, we obtain
\begin{align*}
	m_1{u} & = m_1{v}_1\cos\alpha +
	\dfrac{m_1{v_1}\sin\alpha}{\sin\beta}\cos\beta \\
\end{align*}
\begin{equation}
	{u} = {v}_1\ab(\cos\alpha + \sin\alpha\cot\beta)
	\label{eq:uv1}
\end{equation}

Similarly, from \cref{eq:approachspeed}, we can also write
\begin{equation}
	{v}_2 = \dfrac{{u} + {v}_1\cos\alpha}{\cos\beta}
	\label{eq:v2}
\end{equation}

Substituting \cref{eq:v2} into \cref{eq:mv2}, we obtain
\begin{align*}
	m_2 & = \dfrac{m_1{v}_1\sin\alpha}{\sin\beta} \cdot \dfrac{1}{{v}_2} \\
	    & =
	\dfrac{m_1{v}_1\sin\alpha}{\sin\beta}\cdot\dfrac{\cos\beta}{{u}
	+ {v}_1\cos\alpha}                                                   \\
\end{align*}
\begin{equation}
	\therefore\; m_2
	=\dfrac{m_1{v}_1\sin\alpha\cot\beta}{{u}+{v}_1\cos\alpha}
	\label{eq:m2}
\end{equation}
...
The final velocity of the rocket, \({v}_1\), is
\begin{align*}
	{v}_1 & = \sqrt{{v}_0^2 + 2{a}{s}}              \\
...
\begin{align*}
	...
	\mv{v}_C                          & = -3\mv{v}_1 - \ab(\mv{v}_A + \mv{v}_B)        \\
	                                  ...
	                                  & = -240\ii - (3v_1 + 450)\jj                    \\
	{v}_C                             & = \sqrt{\ab(-240)^2 + \ab[-(3v_1+450)]^2}      \\
...
\(e\) of the vertical velocity before impact: \({v_y}' = ev_y\).
...
In the vertical direction, \(v_y = \sqrt{{u_y}^2 +
	2as}\). With \(u_y = 0\)
at the ball's maximum height,
\begin{align*}
	v_y & = \sqrt{2\ab(-g)\ab(-0.20)} \\
	    & = \sqrt{0.4g}
\end{align*}

Additionally, for every vertical drop the ball undergoes starting
from the edge of the previous step to the platform of the next,
\(v_y = \sqrt{u_y^2 + 2as}\) also.
\begin{align*}
	u_y & = \sqrt{v_y^2 - 2as}              \\
...
Then \(e\) is just the ratio between the two quantities!
\begin{align*}
	e & = \lf{v_y}{u_y}                   \\
...
and time of ascent of the ball. Since \(v = u + at\), the time \(t\)
...
If we treat the ball as a projectile, the horizontal component
of its velocity \(v_x\) is constant throughout. Therefore,
\(s_x = v_xt\):
\begin{align*}
	v_x & = \lf{s_x}{t}                               \\
	    & = \f{s_x\sqrt{g}}{\sqrt{0.4} + \sqrt{0.8}}  \\
	    & = \f{0.30\sqrt{g}}{\sqrt{0.4} + \sqrt{0.8}} \\
	    & = \hil{\qty{0.62}{\metre\per\second}}
\end{align*}

\begin{problem}
At the bottom of the steps, the ball continues to bounce along a
horizontal pavement. The collisions of the ball with the pavement
have the same value of \(e = \sqrt{\lf{1}{2}}\).
Show that the ball does not bounce beyond a distance from the
bottom of the steps, and find this distance.
\end{problem}

After the \(n\)--th bounce, the vertical component of the ball's velocity
will be \({v_y}_n = e^{n-1}{v_y}_0\), where \({v_y}_0\) is the ball's vertical
velocity after the first bounce.
\begin{align*}
	{v_y}_0 & = \sqrt{2gh}
\end{align*}
...
From before, the horizontal distance travelled by the ball on its
first bounce is
\begin{align*}
	{s_x}_0 & = v_xt_0             \\
	        & = \f{2v_x{v_y}_0}{g}
...
By extension, the additional horionztal distance travelled on the
\(n\)--th bounce can be found by substituting \({v_y}_n\):
\begin{align*}
	{s_x}_n & = \f{2v_x{v_y}_n}{g}        \\
	        & = \f{2e^{n-1}v_x{v_y}_0}{g}
\end{align*}

The total horizontal distance travelled will be
the sum \(s = {s_x}_0 + {s_x}_1 + \cdots + {s_x}_n\).
This is a geometric series, for which \(n\to\infty\).
\begin{align*}
	s & = \f{2v_x}{g}\ab({v_y}_0 + {v_y}_1 + \cdots + {v_y}_n)                \\
	  & = \f{2v_x{v_y}_0}{g}\ab(1 + e + e^2 + \cdots + e^{n-1})               \\
	  & = \f{2v_x{v_y}_0}{g}\f{1}{1-e}                                        \\
	  & = \f{2}{g\ab(1-e)}\f{0.30\sqrt{g}}{\sqrt{0.4}+\sqrt{0.8}}\sqrt{0.40g} \\
	  & = \hil{\qty{.863}{\metre}}
\end{align*}

The maximum distance travelled by the ball after its descent is
\hil{\qty{0.863}{\metre}}.
